...
public class DesignerVariableEditorPage extends EditorPart implements
	...
	public void createPartControl(Composite parent) {
		...
		form.setText(Messages
				.getString("DesignerVariableEditorPage.label.variables")); //$NON-NLS-1$
		...
		allVariableSelection.setText(Messages
				.getString("DesignerVariableEditorPage.label.all_variables")); //$NON-NLS-1$
		...
		addButton(
				...
				Messages.getString("DesignerVariableEditorPage.label.create"), new CreateVariableSelectionListener(parent)); //$NON-NLS-1$
		addButton(
				toolkit,
				businessButtonComposite,
				Messages.getString("DesignerVariableEditorPage.label.remove"), new RemoveVariableSelectionListener()); //$NON-NLS-1$

		Section stateVariableSelection = toolkit.createSection(sashForm,
				Section.TITLE_BAR);
		stateVariableSelection.setText(Messages
				.getString("DesignerVariableEditorPage.label.state_variables")); //$NON-NLS-1$
		...
		stateSelection.setText(Messages
				.getString("DesignerVariableEditorPage.label.states")); //$NON-NLS-1$
	...
	private void createStateVariableList(FormToolkit toolkit,
		...
		// Table table = new Table(masterClient,SWT.SINGLE);
			...
			for (int i = 0; i < stateTable.getItemCount(); i++) {
				...
				if (stateVariableList.contains(tableItem.getData())) {
					tableItem.setImage(SharedImages.getImage(SharedImages
							.getImageDescriptor("/icons/selected_state.gif")));;
				} else {
