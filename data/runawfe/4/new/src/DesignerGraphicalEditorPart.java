...
public class DesignerGraphicalEditorPart extends GraphicalEditorWithFlyoutPalette implements RequestConstants {
	...
	public DesignerGraphicalEditorPart(DesignerEditor editor) {
		...
		IAction copyAction = new CopyAction(Messages.getString("Action.copy"));
		copyAction.setId("copy");
		//copyAction.setAccelerator(SWT.CTRL + 'C');
		IAction pasteAction = new PasteAction(Messages.getString("Action.paste"));
	...
	// setActionRegistry(new DesignerActionRegistry(this));
	...
	protected KeyHandler getCommonKeyHandler() {
		if (commonKeyHandler == null) {
			...
			commonKeyHandler.put(KeyStroke.getPressed('c', SWT.CTRL), getActionRegistry().getAction("copy"));
			commonKeyHandler.put(KeyStroke.getPressed('C', SWT.CTRL), getActionRegistry().getAction("copy"));
			commonKeyHandler.put(KeyStroke.getPressed('v', SWT.CTRL), getActionRegistry().getAction("paste"));
			commonKeyHandler.put(KeyStroke.getPressed('V', SWT.CTRL), getActionRegistry().getAction("paste"));
		}
	...
	private class PasteAction extends Action {
		private PasteAction(String text) {
			super(text);
		}
		...
		public void run() {
			try {
				...
				for (Iterator iter = selection.iterator(); iter.hasNext();) {
					...
					NodeGraphicalEditPart editPart = (NodeGraphicalEditPart) object;
