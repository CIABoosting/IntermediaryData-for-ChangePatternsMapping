...
public class DesignerSwimlaneEditorPage extends EditorPart implements
		PropertyChangeListener {

	private static final String DEFAULT_LABEL = Messages
			.getString("DesignerSwimlaneEditorPage.label.default_text"); 
	...
	public void createPartControl(Composite parent) {
		...
		form.setText(Messages
				.getString("DesignerSwimlaneEditorPage.label.swimlanes")); 
		...
		master.setText(Messages
				.getString("DesignerSwimlaneEditorPage.label.all_swimlanes")); 
		...
		detail
				.setText(Messages
						.getString("DesignerSwimlaneEditorPage.label.swimlane_details")); 
		...
		descriptionLabel = toolkit.createLabel(detailClient, DEFAULT_LABEL,
				SWT.WRAP);
	...
	private void addButtons(FormToolkit toolkit, Composite masterClient) {
		...
		Button addButton = toolkit.createButton(masterClient, Messages
				.getString("DesignerSwimlaneEditorPage.label.add"), SWT.PUSH); 
		...
		addButton.addSelectionListener(new SelectionListener() {
			public void widgetSelected(SelectionEvent e) {
				...
				if (dialog.open() != Window.CANCEL) {
					...
					String swimlaneName;
					if ((wizard.getSwimlaneName() != null)
							|| !("".equals(wizard.getSwimlaneName())))
						swimlaneName = wizard.getSwimlaneName();
					else
						swimlaneName = editor.getProcessDefinition()
								.getNextSwimlaneName();
					command.setName(swimlaneName);
					if (wizard.getSwimlaneDescription() != null
							|| !("".equals(wizard.getSwimlaneDescription())))
						command.setDescription(wizard.getSwimlaneDescription());
					command.setProcessDefinition(editor.getProcessDefinition());
					String configuration = createSwimlaneConfiguration(
							swimlaneName, wizard.getType(), wizard.getParams(),
							wizard.isAddReplacement());
		...
		Button removeButton = toolkit
				.createButton(
						masterClient,
						Messages
								.getString("DesignerSwimlaneEditorPage.label.remove"), SWT.PUSH); 
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if (ProcessDefinition.ELEMENT_SWIMLANE_ADDED.equals(type)
				|| ProcessDefinition.ELEMENT_SWIMLANE_REMOVED.equals(type)) {
			fillViewer();
		} else if (evt.getSource() instanceof Swimlane
				&& Swimlane.PROPERTY_NAME.equals(type)) {
			viewer.refresh(evt.getSource());
		} else if (evt.getSource() instanceof Swimlane
				&& Swimlane.PROPERTY_DESCRIPTION.equals(type)) {
			IStructuredSelection selection = (IStructuredSelection) viewer
					.getSelection();
	...
	private String createSwimlaneConfiguration(String swimlaneName,
			String typeName, List params, boolean addReplacement) {
		...
		result += ")";
		if (addReplacement) {
			result += ";ru.runa.af.organizationfunction.TableSubstitutionFunction(?,"
					+ editor.getProcessDefinition().getName()
					+ "."
					+ swimlaneName
					+ "):"
